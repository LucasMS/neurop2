...
## Perform test
```{r}
res <- bind_rows(test.deseq(micro, meta, asv.to.keep, "Untreated"),
                 test.deseq(micro, meta, asv.to.keep, "AtopiControl")) %>% 
...
micro.st <- micro.r %>% 
  ...
  mutate(Visit.plot = if_else(Treatment == "AtopiControl",
                              paste0("  ", Visit),
                              paste0(" ", Visit)))
...
label <- res %>% 
  ...
  mutate(ASV = paste0(Species, " ", gsub("ASV", "", ASV))) %>% 
  mutate(ASV = factor(ASV, levels = unique(ASV))) %>% 
  mutate(hjustvar = if_else(Treatment == "AtopiControl",
                            0,
                            1.1),
         xpos = if_else(Treatment == "AtopiControl",
                            -Inf,
                            Inf))
```
## Plot
